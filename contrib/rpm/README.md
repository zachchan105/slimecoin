...
The RPM spec file provided here is for Slimecoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://slimecoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Slimecoin git repository](https://github.com/SlimecoinProject/Slimecoin) but are not part of
...
Currently only `Source10` is used, for the example `slimecoin.conf` file.
...
in the slimecoin git repository. At present only an SVG version of the Slimecoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the slimecoin
...
standard SLM library on most Linux distributions. The patch will likely not be
needed when 0.12.1 is released, a proper fix is already in the Slimecoin git
master branch.
...
idea to build Slimecoin Core against a static version of that library compiled
...
The `slimecoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `slimecoin-server` package. At this point, most RPM based
...
The `slimecoin-server` package should have SELinux support. How to properly do
...
Slimecoin-Core against OpenSSL or use a patched version of Boost in the build
system.

As SSLv3 is no longer safe, distributions that have not patched Boost to work
with SLM libraries that do not support SSLv3 should have bug reports filed
...
[Ringing Liberty](https://www.ringingliberty.com/slimecoin/). He has been
packaging Slimecoin for Fedora at least since 2012.
...
4. I renamed the `slimecoin` package that contains the Qt GUI to `slimecoin-core` as
that appears to be how the general population refers to it, in contrast to
`slimecoin-xt` or `slimecoin-classic`. I wanted to make sure the general population
