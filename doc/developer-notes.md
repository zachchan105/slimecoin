...
The Slimecoin Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play slimecoins" on the test network, if you
...
Slimecoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 4767.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 4766 for connections and services them.

- SlimecoinMiner : Generates slimecoins (if wallet is enabled).
...
However, in open source software such as Slimecoin Core, where everyone uses
...
pay attention to for reviewers of Slimecoin Core code.

General Slimecoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Slimecoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Slimecoin so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:slimecoin/slimecoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `slimecoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `slimecoin-cli` and the GUI debug console use this table to determine how to
