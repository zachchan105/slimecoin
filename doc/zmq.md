...
The Slimecoin Core daemon can be configured to act as a trusted "border
router", implementing the slimecoin wire protocol and relay, making
...
The ZeroMQ feature in Slimecoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building slimecoind:
...
For instance:
    ...
    $ slimecoind -zmqpubhashtx=tcp://127.0.0.1:24766 \
               -zmqpubrawtx=ipc:///tmp/slimecoind.tx.raw
...
These options can also be provided in slimecoin.conf.
...
From the perspective of slimecoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into slimecoind directly. Furthermore, no information is
...
using. Slimecoind appends an up-counting sequence number to each
