...
`test/functional/test_framework/test_framework.py:SlimecoinTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/slimecoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and slimecoind
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_slimecoind = config["components"].getboolean("ENABLE_SLIMECOIND")
    if not (enable_wallet and enable_cli and enable_slimecoind):
        print("No functional tests to run. Wallet, utils, and slimecoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if slimecoind is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "slimecoind"]) is not None:
            print("%sWARNING!%s There is already a slimecoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "SLIMECOIND" not in os.environ:
        os.environ["SLIMECOIND"] = build_dir + '/src/slimecoind' + exeext
        os.environ["SLIMECOINCLI"] = build_dir + '/src/slimecoin-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `slimecoin-cli help` (`rpc_interface.txt`).
