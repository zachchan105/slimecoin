...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for SLM
    ...
    if (p_rewardSnapshot.strDistributionAsset != "SLM") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is SLM with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using SLM differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "SLM") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Slimecoin address (already validated during ownership snapshot creation)
